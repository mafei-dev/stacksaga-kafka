...
@Endpoint(eventSuffix = "UPDATE_STOCK")
public class UpdateStockEndpoint extends CommandEndpoint {

    @Override
    @KafkaListener("DO_UPDATE_STOCK")
    public void doProcess(ConsumerRecord<String, JsonNode> aggregator, @Headers Map<String, Object> headers) throws SagaRetryableException {
        if (aggregator.value() instanceof ObjectNode node) {
            ...
            node.put("DO_UPDATE_STOCK", LocalDateTime.now().toString());
        }
    }

    @Override
    @KafkaListener("UNDO_UPDATE_STOCK")
